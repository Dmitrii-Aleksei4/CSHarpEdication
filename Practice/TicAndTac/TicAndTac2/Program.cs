                ...
                switch (answerStart)
                {
                    ...
                    case "yes":
...

                        {
                            ...
                            if (koordinatX_O.Length != 2) { Console.WriteLine("Мало или много символов"); continue; }
                            
                            if (!lenMas.Contains(koordinatX_O[0].ToString()))
                      
                                { Console.WriteLine("Неверные первые координаты"); continue; }
                            
                            if (koordinatX_O[1] != '1' && koordinatX_O[1] != '2' && koordinatX_O[1] != '3')
                            { Console.WriteLine("Неверные вторые координаты"); continue; }

                            // переводим координаты из чар в int
                            koorX = (koordinatX_O[0] - '0') - 1;
                            koorY = (koordinatX_O[1] - '0') - 1;
           ...
           
            {
                ...
                for (int i = 0; i < battle.GetLength(0); i++)
                {
                    for (int j= 0; j < battle.GetLength(1); j++)
                    {
                        ...
                        Console.BackgroundColor = ConsoleColor.Black;
                        if (j < battle.GetLength(0) - 1) Console.Write("|");
                    }
                    if (i < battle.GetLength(0)-1) Console.WriteLine("\n-----------------");
                }
