...
namespace CodenamesCore.GameLogic
{
    public class MethodsGames
    {
        ...
        /// <returns></returns>
        public BattleGame GetAllDiktWords(List<string> getAllDiktWords, int sizeBattleHight = 5, int sizeBattleWidht = 5)
        {
            ...
            RolesSpies color;
            var (numberColorAgent, scoreAgents) = AllNumberAgent(higth*widht);
            ...
            var readyBattle = new BattleGame(battle, scoreAgents);
        ...
        public (List<RolesSpies>,Dictionary<RolesSpies,int>) AllNumberAgent(int sizeNumber)
        {
            int corectorPart = 0;
            ...
            var scoreAgents = new Dictionary<RolesSpies, int>()
            {
                {RolesSpies.blue,blue },
                {RolesSpies.red,red },
                {RolesSpies.black,black },
                {RolesSpies.white,white },
            };
            


            return (agetns, scoreAgents) ;
        }
        ...
        public async Task StartTimerAsync(int currentX, int currentY)
        {
            Console.WriteLine("Таймер запущен на 120 секунд");
            for (var i = 0; i < 120; i++) 
            {
                Console.SetCursorPosition(currentX, currentY);
                await Task.Delay(1000); // 120 секунд = 120000 мс
                Console.WriteLine(i);
            }
