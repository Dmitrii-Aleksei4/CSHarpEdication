    ...
    public partial class BattleCapitane
    {
        ...
        private async Task LoadDataFromCache()
        {
            try
            {
                ...
                // 1. Получаем параметр из URL
                var uri = new Uri(NavigationManager.Uri);
                var query = System.Web.HttpUtility.ParseQueryString(uri.Query);
                string encoded = query["text"];

                // 2. Если есть параметр - используем его как ключ
                if (!string.IsNullOrEmpty(encoded))
                {
                    key = Uri.UnescapeDataString(encoded);
                    Console.WriteLine($"✅ Ключ из URL: {key}");
                }
                else
                {
                    // Если параметра нет - пробуем загрузить сохраненный ключ
                    string savedKey = await Js.InvokeAsync<string>("localStorage.getItem", "remembered_key");
                    if (!string.IsNullOrEmpty(savedKey))
                    {
                        key = savedKey;
                        Console.WriteLine($"✅ Используем сохраненный ключ: {key}");
                    }
                    else
                    {
                        errorMessage = "⚠️ Ключ не найден. Используйте ?text=ваш_ключ";
                        Cards = new List<List<Card>>();
                        return;
                    }
                }

                // 3. Загружаем данные из localStorage
                string loadedJson = await Js.InvokeAsync<string>("localStorage.getItem", key);

                // 4. Проверяем, есть ли данные
                if (!string.IsNullOrEmpty(loadedJson))
                {
                    Cards = new List<List<Card>>();
                    battleGame = JsonSerializer.Deserialize<BattleGame>(loadedJson);
                    Console.WriteLine(battleGame.NameCommand[0]);
                    Console.WriteLine(battleGame.RulesAgents[RolesSpies.blue].ToString());
                    //Console.WriteLine($"✅ Данные загружены! Карт: {Cards?.Count ?? 0}");
                ...
                {
                    errorMessage = $"⚠️ По ключу '{key}' данных нет";
                    Cards = new List<List<Card>>();
                }
            }
            catch (Exception ex)
